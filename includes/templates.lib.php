...
function volcar_template($ruta,$contingut,$retorn=false)
{
	if($ruta==''){die('Falta ruta'); }
	$tpl = new RainTPL;

	foreach($contingut as $k=>$v){
		$tpl->assign($k,$v);
	}
	
	if(empty($_SERVER['HTTP_X_REQUESTED_WITH']) && !$retorn && $ruta!='logged' && $ruta!='notlogged' ) 
	{
		$subContent= $tpl->draw($ruta,true);
		include_once($GLOBALS['PATH_ARRAY']["fisico"].'logged.php');
		die();
	}
	
	if($retorn){
		return $tpl->draw($ruta,true);
	}else{
		$tpl->draw($ruta,false);
	}
}
...
class RainTPL{
	...
	private function check_template( $tpl_name ){
		...
		if( !isset($this->tpl['checked']) ){
			...
			$finalUrl                            = $GLOBALS['PATH_ARRAY']['templates'] . $tpl_basedir;								// template directory
			$this->tpl['tpl_filename']          = $finalUrl . $tpl_basename . '.' . self::$tpl_ext;	// template filename
			$temp_compiled_filename             = $GLOBALS['PATH_ARRAY']["cache"] . $tpl_basename . "." . md5( $finalUrl . self::$config_name_sum );
			...
			if( !file_exists( $this->tpl['compiled_filename'] ) || ( self::$check_template_update && filemtime($this->tpl['compiled_filename']) < filemtime( $this->tpl['tpl_filename'] ) ) ){
				$this->compileFile( $tpl_basename, $tpl_basedir, $this->tpl['tpl_filename'], $GLOBALS['PATH_ARRAY']["cache"], $this->tpl['compiled_filename'] );
				return true;
			}
	...
	*/
	...
	private function compileFile( $tpl_basename, $tpl_basedir, $tpl_filename, $cache_dir, $compiled_filename ){
		...
		// create directories
		if( !is_dir( $GLOBALS['PATH_ARRAY']["cache"] ) )
			mkdir( $GLOBALS['PATH_ARRAY']["cache"], 0755, true );

		if( !is_writable( $GLOBALS['PATH_ARRAY']["cache"] ) )
			throw new RainTpl_Exception ('Cache directory ' . $GLOBALS['PATH_ARRAY']["cache"] . 'doesn\'t have write permission. Set write permission or set RAINTPL_CHECK_TEMPLATE_UPDATE to false. More details on http://www.raintpl.com/Documentation/Documentation-for-PHP-developers/Configuration/');
			...
			elseif( preg_match( '/(?:\{include="([^"]*)"(?: cache="([^"]*)"){0,1}\})/', $html, $code ) ){
				...
				if( isset($code[ 2 ]) )
					...
					$compiled_code .= '<?php $tpl = new RainTPL;' .
								 ...
								 '	echo $cache;' .
								 'else{ ' .
								 '$GLOBALS[\'PATH_ARRAY\'][\'templates\']_temp = $GLOBALS[\'PATH_ARRAY\'][\'templates\'];' .
								 '$tpl->assign( $this->var );' .
	...
	private function path_replace( $html, $tpl_basedir ){
		...
		if( self::$path_replace ){
                        ...
                        // reduce the path
                        $path = preg_replace('/\w+\/\.\.\//', '', self::$GLOBALS['PATH_ARRAY']["normal"] . self::$GLOBALS['PATH_ARRAY']['templates'] . $tpl_basedir );

			$exp = $sub = array();

			if( in_array( "img", self::$path_replace_list ) ){
				$exp = array( '/<img(.*?)src=(?:")(http|https)\:\/\/([^"]+?)(?:")/i', '/<img(.*?)src=(?:")([^"]+?)#(?:")/i', '/<img(.*?)src="(.*?)"/', '/<img(.*?)src=(?:\@)([^"]+?)(?:\@)/i' );
				$sub = array( '<img$1src=@$2://$3@', '<img$1src=@$2@', '<img$1src="' . self::$GLOBALS['PATH_ARRAY']["normal"] . self::$GLOBALS['PATH_ARRAY']['templates'] . $tpl_basedir . '$2"', '<img$1src="$2"' );
			}
			...
			if( in_array( "script", self::$path_replace_list ) ){
				...
				$sub = array_merge( $sub , array( '<script$1src=@$2://$3@', '<script$1src=@$2@', '<script$1src="' . self::$GLOBALS['PATH_ARRAY']["normal"] . self::$GLOBALS['PATH_ARRAY']['templates'] . $tpl_basedir . '$2"', '<script$1src="$2"' ) );
			}
			...
			if( in_array( "link", self::$path_replace_list ) ){
				...
				$sub = array_merge( $sub , array( '<link$1href=@$2://$3@', '<link$1href=@$2@' , '<link$1href="' . self::$GLOBALS['PATH_ARRAY']["normal"] . self::$GLOBALS['PATH_ARRAY']['templates'] . $tpl_basedir . '$2"', '<link$1href="$2"' ) );
			}

			if( in_array( "a", self::$path_replace_list ) ){
				$exp = array_merge( $exp , array( '/<a(.*?)href=(?:")(http|https)\:\/\/([^"]+?)(?:")/i', '/<a(.*?)href="(.*?)"/', '/<a(.*?)href=(?:\@)([^"]+?)(?:\@)/i'  ) );
				$sub = array_merge( $sub , array( '<a$1href=@$2://$3@', '<a$1href="' . self::$GLOBALS['PATH_ARRAY']["normal"] . '$2"', '<a$1href="$2"' ) );
			}
			...
			if( in_array( "input", self::$path_replace_list ) ){
				...
				$sub = array_merge( $sub , array( '<input$1src=@$2://$3@', '<input$1src=@$2@', '<input$1src="' . self::$GLOBALS['PATH_ARRAY']["normal"] . self::$GLOBALS['PATH_ARRAY']['templates'] . $tpl_basedir . '$2"', '<input$1src="$2"' ) );
			}
